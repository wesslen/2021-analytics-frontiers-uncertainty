...
meta() %>%
  ...
  meta_social(
    ...
    image = "https://avatars.githubusercontent.com/u/7621432?v=4",
...
i think it's safe to say the one word we all have gotten to know too well recently is "uncertainty". whether it is a once in a century pandemic, volatile stock market swings, turbulent political elections, or even unexpected giant ships that blocks a critical global trade route, uncertainty is all around us.
...
* alea-toric (inherent noise/randomness) or ep-is-tem-ic (lack sufficient data) or ontological (uncertainty how accurately a model describes reality)
...
simple headlines and bullet points that rely solely on point estimates may obfuscate what we don't know about the data to the general public
...
[Accuracy on Named Entity Recognition (NER) on CoNLL-2003 over time: Sebastian Ruder](https://ruder.io/state-of-transfer-learning-in-nlp/)

???

or even in AI like NLP where state-of-the-art model accuracies like this -- a timeline of accuracy in one natural language processing task -- named entity recognition -- on a popular benchmark dataset.

for machine learning, how can we decide with confidence if one model really is better than another one without explicitly considering uncertainty?
...
so for this talk, i want to outline a few of these concepts. by no means will this be exhaustive and my ultimate hope is to encourage existing or aspiring data scientists to spur curiosity to study these concepts and research more in the future.
...
but first, let me briefly outline the general research approach of these fields in a three step feedback process.

first, we use theories from psychology, cognitive science and behavioral economics to motivate research problems using data visualizations. this could cover low level theories of decision-making like perceptual psychology where we're interested in how people perceive form, shape, and animation through low level cognitive functions. alternatively, this could also cover theories of higher level cogntive functions that require deliberate effort to process difficult information and require a choice of attention and careful allocation of precious cognitive resources.
...
Let me introduce Sherman Kent, or the father of Intelligence Analysis. During the cold war, US intelligence agencies struggled at providing probabilistic statements in intelligence briefings as there were inconsistencies in describing the likelihood of future events.

because of this, Kent established one of the first paradigm to map "words of estimative probability" or WEP's. 
...
In this figure, we can see a list of such words with the dark bands being Kent's probability ranges and dots from a NATO study in which "twenty-three [NATO] officers, ranking from squadron leader to lieutenant general" were asked about how they quantify probabilistic words. 

Officers were asked phrases "It is highly likely that the Soviets will invade Czechoslovakia," or "It is almost certain that the Soviets will invade Czechoslovakia," or "We believe that the Soviets will invade Czechoslovakia." and asked to quantify probabilities they perceived.
...
[Pherson and Pherson, 2013](https://books.google.com/books?id=_X0FHYpHDzoC&pg=PA187&lpg=PA187)

???

We can also look at a more modern version of the same study.

Two things are striking - first even with the guidance of Kent's proposals, many officers had different probabilities. But even more fascinating, is the range of probabilities for different words.
...
We can also improve these results into density ridge plots -- ggridges -- which helps us better compare across the distributions per word.

In this way, we can see how for the same word, we could have vary different perceptions of the probabilties. This is critical for data scientists to realize for the need of careful, consistent lanugage in describing probabilities.
...
but there are other challenges with probabilities, especially very small probabilities. let's take an example from the washington post describing the potential predictions of the 2016 presidential election.
...
```{r, out.width = "350px"}
knitr::include_graphics("./img/icon-array.png")
```

#### Icon arrays and Isotypes by Otto Neurath

["Understanding uncertainty: Visualizing Probabilities"](https://plus.maths.org/content/understanding-uncertainty-visualising-probabilities) by Pearson & Short

???

or related we can use icons or "isotypes" to encode probabilities as shown in this classic chart by the austrian philosophy Ott Neurath who popularized the use of icons as a means to communicate information.

this example, provides women labor force participation rates in 1930 by country. with the frequency framing, it's easier to take away how more frequently women were  
...
alternatively, there are also great examples of excellent data journalism communication by media outlets like the New York Times.

in a recent article, they used frequency framing to help readers better interpret vaccine effectiveness.

using the Johnson & Johnson vaccine, we may hear that it has a 66% efficacy -- but what does that mean?

using frequency framing, they displayed individual dots for the two experimental groups. in the placebo group, 49 per every 5,000 participants contracted covid-19 -- however, at the same time only 17 per every 5,000 participants who received the vaccination contracted covid-19. in this way, we can think of the efficacy as a relative measure of -- holding all things constant -- how much lower the chance of contracting covid after being vaccinatinated with the J&J vaccine compared to not getting the vaccine.

visually, we can view 66% fewer green dots for the vaccinated group compared to the blue dots for the placebo dots.

this is even more important, where if we view similar relative probabilities but for contracting severe covid, in which we have an 85% less risk of severe cases.
...
but even if we understand efficacy -- what can we say about the uncertainty around that value?

well - many of you may think - ah, that easy, we can calculate my classic 95% confidence interval. well, to paraphrase inigo montoya, perhaps that may not mean what you think it means.

---

class: center

<img src="./img/ci.png" width=100% >

???

if we return to the same article, we can view the 95% confidence intervals for four different vaccine trials.

where the green dot represents our point estimate of the efficacy estimate and the two black dots provide the 95% confidence intervals for that efficacy.

Such intervals are convenient because they allow us to show many estimates with their uncertainties all at once. 

however, what do confidence intervals really mean?

--

<img src="./img/ci-phrase.png">


???

well first, you may think that it means there is a 95% chance that the true value falls into the confidence interval? and unfortunately that's not really true.
...
but instead -- as highlighted by Lucy McGowan of Wake Forest university -- highly recommend to follow her and her awesome stats podcast "casual inference" -- quotes the true definition of a classical confidence interval:
...
```{r, out.width = "200px"}
knitr::include_graphics("./img/hops.gif")
```
...
this plot can be created in two steps. (1) Draw a sample of hypothetical outcomes (draws) from the distribution; (2) for each, make a plot that becomes one frame in an animated presentation. 
...
if you remember, Frequentists refer to interpretating of probability as the long-run relative frequency of a repeatable event.
...
as you may remember, frequentist is based on 
...
one common application is making election predictions. these are helpful as we update our beliefs about the elections as new data -- polls -- are updated.
...
it's important to consider three important points.
...
Bill had an amazing transformative career where he helped revolutionize visualization research, first in Georgia Tech where he researched VR and interactive visualizations starting in the early 90s and then helped transform UNCC as a leader in the burdening fields of data visualization when he joined in 2004 and later served as chair of the computer science department as well helping to develop UNCC's data science intiative, which is now the school of data science.

His strong partnerships with industry like Bank of America as well as government agencies like the Department of Homeland Security, EPA, DOE, U.S. Department of Transportation, showed how relevant vis research can be to practioners and we're proud to continue the legacy Bill paved.

This research is joint collaboration with Wenwen Dou, Alireza Karduni, and Doug Markant.
...
to measure individuals beliefs, we developed a d3 javascript techinque in which users could click a line to measure their belief on the correlation between two variables. they could then expand the "cone" to measure their beliefs about how certain they are for their belief.

wider cones indicate more uncertainty around their prediction while more narrow cones indicate more certainty on their predicted correlation.

in doing so, participants were providing their prior beliefs on the correlation estimate as well as a measure
...
```{r, out.width = "900px"}
knitr::include_graphics("./img/line-cone-final-1.png")
```
...
with this new techniques, we designed three crowdsourced mturk experiments where users were asked to do three things.

first, participants would be asked their beliefs on two variables, using the line-cone technique to measure both their prior belief on the estimate as well as their level of uncertainty on that belief. we provided users dozens of political, social and economic variables tested through pilot studies, with a range of charged variables like vaccination rates and rate of illness or more benign like exercise amount and body weight. 

next, we provided participants hypothetical data in typically a scatter plot for data for the elicited variables. the data was fabricated to enable us to control for different rates of variation (positive, zero, negative, high or low variance). in addition to scatter plots, we also tested different uncertainty visualizations too.

third, we elicited participants a 2nd time to provide their posterior beliefs. in effect, what we wanted to measure was how much change did participants modify their prior by "learning" from the data or alternatively -- ignoring the data and sticking to their prior.

in effect, the goal was to measure something akin to confirmation bias and see if and when participants may exhibit more or less bias.
...
```{r, out.width = "900px"}
knitr::include_graphics("./img/line-cone-final-2.png")
```
...
to compare participants updating of their beliefs, we used Bayesian cognitive modeling to provide a normative benchmark for how beliefs should change depending on the strength of the evidence and participants’ uncertainty. 
...
```{r, out.width = "900px"}
knitr::include_graphics("./img/line-cone-final-3.png")
```
...
Last, we compared users predicted posteriors versus their elicited posteriors, measuring the difference by either mean absolute error or KL divergence, which measure the difference across the full distribution.

By doing so this gave us measurement of deviation from ideal rationality, in this sense a "human bias". typically bias is used frequently but it's always critical when someone says bias, you need to always ask: "bias relative to what?". the challenge is typically we can't quantify as we use ideals of perfect behaviors. but this approach provides a long term technique that will enable to measure based on a normative, or as if rational individual.
...
what we want to know is could we replicate their findings in an incentivized mturk study and could more recent uncertainty visualizations like HOPs or dot plots nudge participants to allocate more to stocks. we also included controls like simply showing the values in table as well.
...
[Karduni, Wesslen, Markant, and Dou](https://arxiv.org/abs/2102.13167) (In Review)
]
.pull-right[
<blockquote class="twitter-tweet tw-align-center" data-width="300" data-conversation="none"><p lang="en" dir="ltr">We created an interface to elicit users&#39; judgments and uncertainties using a visual technique. Users would 1) assess how biased tweets from a specific (anonymized) source are and 2) rate how credible that source is. They would also write down their rationale<br><br>5/16 <a href="https://t.co/LAvNUhSYGe">pic.twitter.com/LAvNUhSYGe</a></p>&mdash; Alireza Karduni (@don_kordeone) <a href="https://twitter.com/don_kordeone/status/1366567001313918982?ref_src=twsrc%5Etfw">March 2, 2021</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
]
